...
sums :: Int -> [Int]
sums i = sums' 1 1 where sums' j curr = if j>i then [] else curr: sums' (j+1) (curr+j+1)
...
sumsOf :: [Int] -> [Int]
sumsOf xs = sumsOf' xs 0 where
    sumsOf' [] curr = []
    sumsOf' (x:xs) curr = (curr+x): sumsOf' xs (curr+x)
