...
visit maze place = do
  visited <- gets (elem place)
  unless visited $ do
    modify (place :)
    let neighbors = case lookup place maze of
                      Just ns -> ns
                      Nothing -> []
    forM_ neighbors (visit maze)
...
allSums :: [Int] -> [Int]
allSums xs = do
  subset <- filterM (const [True, False]) xs
  return (sum subset)
...
f1 k acc x = let sum = acc + x in
             if sum > k then Nothing else Just sum
...
f2 acc x = do
  seen <- gets (elem x)
  when (not seen) $ modify (x :)
  gets (if seen then const acc else (+ acc) . head)
