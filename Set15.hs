...
calculator op s = operation <*> readMaybe s
  where
    operations = [("negate", negate), ("double", (*2))]
    operation = lookup op operations
...
validateDiv a b = if b /= 0 then pure (a `div` b) else invalid "Division by zero!"
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
  (:) <$> liftA3 Person name1 age1 employed1 <*> ((:[]) <$> liftA3 Person name2 age2 employed2)
...
boolOrInt s = case (boolVal, intVal) of
                (Just b, _) -> pure (Left b)
                (_, Just i) -> pure (Right i)
                (Nothing, Nothing) -> invalid boolErrMsg *> invalid intErrMsg
  where 
    boolVal = readMaybe s :: Maybe Bool
    intVal = readMaybe s :: Maybe Int
    boolErrMsg = "Not a Bool"
    intErrMsg = "Not an Int"
...
normalizePhone s = 
  let noSpaces = filter (not . isSpace) s
      tooLong = if length noSpaces > 10 then [invalid "Too long"] else [pure ()]
      nonDigits = [invalid ("Invalid character: " ++ [c]) | c <- noSpaces, not (isDigit c)]
  in foldr (*>) (pure ()) (tooLong ++ nonDigits) *> pure noSpaces
...
instance Applicative Priced where
  pure a = Priced 0 a
  (Priced price1 f) <*> (Priced price2 a) = Priced (price1 + price2) (f a)
...
myFmap f x = myLiftA2 (\f' a -> f' a) (myPure f) x
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap h (Both fga) = Both (fmap (\ga -> fmap h ga) fga)
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  Both fga <*> Both fga' = Both ((liftA2 (<*>) fga fga'))
