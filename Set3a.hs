...
mapMaybe2 f (Just xval) (Just yval) = Just $ f xval yval
...
capitalize arr = unwords [(\(c:str) -> toUpper c : str) str | str <- words arr]
...
powers :: Int -> Int -> [Int]
powers k max = powers' k max 0 where
    powers' k max curr
        | k^curr <= max = k^curr : powers' k max (curr+1)
        | otherwise = []
...
joinToLength len str = [s1++s2 | s1 <- str, s2 <- str, len == length (s1 ++ s2)]
...
sumRights li = sumRights' $ rights li where
    sumRights' [] = 0
    sumRights' (l:li) = sumRights' li + l
...
interpreter commands = interpreter' commands [0, 0] [] where
    interpreter' [] [x,y] res = res
    interpreter' (c:cs) [x,y] res = case c of
        "up" -> interpreter' cs [x, y+1] res
        "down" -> interpreter' cs [x, y-1] res
        "left" -> interpreter' cs [x-1, y] res
        "right" -> interpreter' cs [x+1, y] res
        "printX" -> interpreter' cs [x,y] (res ++ [show x])
        "printY" -> interpreter' cs [x,y] (res ++ [show y])
