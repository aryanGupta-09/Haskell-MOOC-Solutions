...
checkCapitals (a, b) 
  | not (null a) && not (null b) && isUpper (head a) && isUpper (head b) = Just (a, b)
  | otherwise = Nothing
...
safeIndex xs i = if i >= 0 && i < length xs then Just (head (drop i xs)) else Nothing
...
countAndLog p xs = Logger (map show fulfilled) (length fulfilled)
  where
    fulfilled = filter p xs
...
balance accountName = BankOp $ \(Bank accounts) ->
  case Map.lookup accountName accounts of
    Nothing -> (0, Bank accounts)
    Just bal -> (bal, Bank accounts)
...
rob from to = BankOp $ \(Bank accounts) ->
  case Map.lookup from accounts of
    Nothing -> ((), Bank accounts)  -- If the 'from' account does not exist, do nothing
    Just amount -> runBankOp (withdrawOp from amount +>> depositOp to amount) (Bank accounts)
...
paren c = state $ \x -> ((), if x == -1 then -1 else case c of
  '(' -> x + 1
...
count x = do
  ...
  case lookup x xs of
    Nothing -> put $ (x, 1) : xs
    Just n  -> put $ (x, n + 1) : filter ((/= x) . fst) xs
