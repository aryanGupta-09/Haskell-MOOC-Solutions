...
openDatabase name = open name >>= \conn -> execute_ conn initQuery >> return conn
...
deposit conn account amount = execute conn depositQuery (account, amount) >> return ()
...
parseCommand (x:name:[]) 
  | x == T.pack "balance" = Just $ Balance name
parseCommand (x:name:amount:[]) 
  | x == T.pack "deposit" && isJust (parseInt amount) = Deposit name <$> parseInt amount
  | x == T.pack "withdraw" && isJust (parseInt amount) = Withdraw name <$> parseInt amount
...
perform conn maybeCommand = 
  maybe (return $ T.pack "ERROR") performCommand maybeCommand
  where
    performCommand (Deposit name amount) = deposit conn name amount >> return (T.pack "OK")
    performCommand (Balance name) = fmap (T.pack . show) (balance conn name)
    performCommand (Withdraw name amount) = deposit conn name (-amount) >> return (T.pack "OK")
...
server db request respond = 
  fmap (responseLBS status200 [] . encodeResponse) (perform db $ parseCommand $ pathInfo request) >>= respond
