...
countValid [] = 0
countValid nos = i + countValid (tail nos)
  where i = if length (head nos) < 6 then 0 else if (head nos !! 2) == (head nos !! 4) || (head nos !! 3) == (head nos !! 5) then 1 else 0
...
repeated [] = Nothing
repeated (x:[]) = Nothing
repeated (x1:x2:xs) = if x1 == x2 then Just x1 else repeated (x2:xs)
...
sumSuccess (x:xs) = case x of
  Left _ -> sumSuccess xs
  Right n -> Right (n + sumSuccess' xs)
  where sumSuccess' [] = 0
        sumSuccess' (x:xs) = case x of
          Left _ -> sumSuccess' xs
          Right n -> n + sumSuccess' xs
...
open code l = case l of
  Opened _ -> Opened code
  Locked c -> if c == code then Opened c else Locked c
...
instance Eq Text where
  Text a == Text b = 
    let nospacetext = filter (not . Data.Char.isSpace) in
    nospacetext a == nospacetext b
...
compose [] _ = []
compose _ [] = []
compose ((fst, snd) : xs) ys = case lookup snd ys of
  Just snd' -> (fst, snd') : compose xs ys
...
permute xs ys = [y | (_, y) <- sortBy (comparing fst) [(x, y) | (x, y) <- zip xs ys]]
