...
cut s1 s2 = Shape (\(Coord x y) -> contains s1 x y && not (contains s2 x y))
------------------------------------------------------------------------------
...
paintSolid color shape (Picture base) = Picture (\(Coord x y) -> if contains shape x y then color else base (Coord x y))
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture (\(Coord x y) -> averageColor [f (Coord (x+dx) (y+dy)) | (dx, dy) <- [(-1, 0), (0, -1), (0, 0), (0, 1), (1, 0)]])

averageColor :: [Color] -> Color
averageColor colors = Color (avg reds) (avg greens) (avg blues)
  where
