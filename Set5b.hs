...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node val l r) = treeSize l + treeSize r + 1
...
treeMax tree = treeMax' tree 0
  where treeMax' Empty curr = curr
        treeMax' (Node val l r) curr = let newCurr = max val curr
                                        in max (treeMax' l newCurr) (treeMax' r newCurr)
...
allValues condition tree = case tree of
  Empty -> True
  Node val l r -> condition val && (allValues condition l && allValues condition r)
...
mapTree f tree = case tree of
  Empty -> Empty
  Node val l r -> Node (f val) (mapTree f l) (mapTree f r)
...
walk [] (Node val l r) = Just val
walk (StepL:steps) (Node val l r) = walk steps l
walk (StepR:steps) (Node val l r) = walk steps r
